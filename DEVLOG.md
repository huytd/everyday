...
A better approach for this case is to use relative epsilon comparison: Calculate the difference between the two numbers `a` and `b`, if the difference is smaller than `n%` of `a`, then `a` and `b` can be considered equal (assuming `a` is the larger number of the two numbers).
